...
import json
from PyQt6.QtWidgets import QMainWindow, QVBoxLayout, QWidget, QGroupBox, QHBoxLayout, QLabel, QLineEdit, QPushButton, QScrollArea, QStackedWidget, QGraphicsBlurEffect
from PyQt6.QtCore import Qt, QSize, QCoreApplication

from classes.ui.menu import add_account_menu  # Import the menu logic from menu.py
from PyQt6.QtWidgets import QMainWindow, QMenuBar, QLabel, QWidget, QVBoxLayout 
from PyQt6.QtGui import QAction,QIcon
from classes.ui.menu import add_account_menu
...
class ComparisonWidget(QWidget):
    def __init__(self, homeWidget, locationWidget, mainWindow):
        ...
        self.searchBar3.returnPressed.connect(self.compareSearchEntered)
    ...
    def updateLabels(self, location1, location2):
        # Update left location name and widgets
        self.leftLocationName.setText(location1.getAddress())
        for varName, widget in self.widgets.items():
            if isinstance(widget, WeatherWidget):
                widget.updateWeatherLabels(location1)
            elif isinstance(widget, MapWidget):
                widget.updateMap(location1)
            elif isinstance(widget, PovertyWidget):
                widget.updateLabels(location1)
            elif isinstance(widget, DescriptionWidget):
                widget.updateLabel(location1)
            elif isinstance(widget, CrimeWidget):
                widget.updateCrime(location1)

        # Fetch data for location2 if needed
        from main import getWeather
        from functions.getCrimeData import getCrimeData
        from functions.getPovertyData import getPovertyData
    
        getCrimeData(location2)
        getPovertyData(location2)
        getWeather(location2)

        # Update right location name and widgets
        self.rightLocationName.setText(location2.getAddress())
        for varName, widget in self.widgets2.items():
            if isinstance(widget, WeatherWidget):
                widget.updateWeatherLabels(location2)
            elif isinstance(widget, MapWidget):
                widget.updateMap(location2)
            elif isinstance(widget, PovertyWidget):
                widget.updateLabels(location2)
            elif isinstance(widget, DescriptionWidget):
                widget.updateLabel(location2)
            elif isinstance(widget, CrimeWidget):
                widget.updateCrime(location2)
            
            # Store comparison data
            if hasattr(self, 'mainWindow') and self.mainWindow:
                self.mainWindow.storeComparisonData(location1, location2)

    def compareSearchEntered(self):
        """Handles the Enter key press in the comparison search bar."""
        print("compareSearchEntered method called.")
        from main import getLocation, getWeather

        global currentLocation  
        if len(self.searchBar3.text()) > 0 and currentLocation:
            comparedLocation = getLocation(self.searchBar3.text())
            print(f"currentLocation: {currentLocation}")
            print(f"comparedLocation: {comparedLocation}")

            if comparedLocation and comparedLocation.getAddress() != "N/A":
                getWeather(comparedLocation)
                self.updateLabels(comparedLocation)

           
                if hasattr(self.mainWindow, "storeComparisonData"):
                    self.mainWindow.storeComparisonData(currentLocation, comparedLocation)
                    print("storeComparisonData was called.")
                else:
                    print("self.mainWindow has no method storeComparisonData.")
            else:
                QMessageBox.warning(self, "Comparison Error", "Could not find the location to compare.")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.child_windows = []  # Track child windows 
        self.user_id = 123123    # Temporary static user ID for wishlist storage
        self.wishlist = get_user_wishlist_data(self.user_id)
        self.wishlist_dialog = None
        self.current_comparisons = []
        ...
        # Automatically resize the window to 90% of the available screen size
        ...
        add_account_menu(self)  # Call the method from menu.py to add the Account menu
        ...
        if currentLocation and currentLocation.getAddress() in self.wishlist:
    ...
    def compareButtonClicked(self):
        ...
        print(f"searchBar has focus: {self.searchBar.hasFocus()}")
    
        
    def storeComparisonData(self, location1, location2):
        """Stores the comparison data of two cities."""
        print("storeComparisonData called")
        if location1 and location2:
            print("Both locations are valid, calling storeComparisonData...")
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            comparison_data = {
                "city1": location1.getAddress(),
                "city2": location2.getAddress(),
                "timestamp": timestamp,
                "metrics": {
                    "temperature (°F)": {
                        location1.getAddress(): location1.getTemperature(),
                        location2.getAddress(): location2.getTemperature()
                    },
                    "precipitation (in last 7 days)": {
                        location1.getAddress(): location1.getPrecipitation(),
                        location2.getAddress(): location2.getPrecipitation()
                    },
                    "state description": {
                        location1.getAddress(): location1.getStateDescription() if hasattr(location1, 'getStateDescription') else "N/A",
                        location2.getAddress(): location2.getStateDescription() if hasattr(location2, 'getStateDescription') else "N/A"
                    },
                    "violent crimes (2023)": {
                        location1.getAddress(): location1.getViolentCrimes() if hasattr(location1, 'getViolentCrimes') else "N/A",
                        location2.getAddress(): location2.getViolentCrimes() if hasattr(location2, 'getViolentCrimes') else "N/A"
                    },
                    "median income": {
                        location1.getAddress(): location1.getMedianIncome() if hasattr(location1, 'getMedianIncome') else "N/A",
                        location2.getAddress(): location2.getMedianIncome() if hasattr(location2, 'getMedianIncome') else "N/A"
                    },
                    "people in poverty": {
                        location1.getAddress(): location1.getPeopleInPoverty() if hasattr(location1, 'getPeopleInPoverty') else "N/A",
                        location2.getAddress(): location2.getPeopleInPoverty() if hasattr(location2, 'getPeopleInPoverty') else "N/A"
                    }
                }
            }

            # Store comparison data in self.current_comparisons
            self.current_comparisons.append(comparison_data)
            ...
            try:
                with open("comparisons.json", "r+") as file:
                    ...
                    # Debug output: print the data already in the file
                    print(f"Existing data in comparisons.json: {data}")
                
                    data.append(comparison_data)
                
                    # Debug output: print the data to be saved in the file
                    print(f"Data to be saved in comparisons.json: {data}")
                
                    file.seek(0)
                    json.dump(data, file, indent=4)
            except FileNotFoundError:
                with open("comparisons.json", "w") as file:
                    # Debug output: file not found, creating a new file
                    print(f"File not found, creating a new file and saving data: {comparison_data}")
                    json.dump([comparison_data], file, indent=4)
        
            # Debug output: print the successfully stored data
            print(f"Comparison data stored successfully: {comparison_data}")
        else:
            print("Error: One or both locations are None, cannot store comparison data.")

    

    def get_current_comparisons(self):
        print(f"Current comparisons: {self.current_comparisons}")
        return self.current_comparisons[:]
    
    
    def open_comparison_dialog(self):
        from classes.ui.comparison_ui import ComparisonUI
        comparison_data = self.get_current_comparisons()
        comparison_dialog = ComparisonUI(self, comparison_data)
        comparison_dialog.exec()
         
    def heartButtonClicked(self):
        global likeLocation, currentLocation

        address = currentLocation.getAddress() if currentLocation else None
        if not address:
            return
        ...
        if likeLocation:
            self.heartButton.setIcon(QIcon("classes/ui/imgs/heart.png"))
            likeLocation = False
            if address not in self.wishlist:
                self.wishlist.append(address)
                print(f"Added to wishlist: {address}")
        else:
            self.heartButton.setIcon(QIcon("classes/ui/imgs/heartEmpty.png"))
            likeLocation = True
            if address in self.wishlist:
                self.wishlist.remove(address)
                print(f"Removed from wishlist: {address}")
        
        save_user_wishlist_data(self.user_id, self.wishlist)

    def show_wishlist_page(self):
        wishlist_dialog = WishlistDialog(self.wishlist.copy(), self.user_id, self) 
        if wishlist_dialog.exec() == QDialog.DialogCode.Accepted:
            self.wishlist = wishlist_dialog.current_data
    ...
    def switch_to_home_page(self):
        """Switch to the home page in the stacked widget."""
        self.stacked_widget.setCurrentWidget(self.centralWidget)
    ...
    def switch_to_second_page(self):
        ...
        # Get the location from appropriate search bar
        ...
        if mode == "new":            
            ...
            # Update all widgets with location data
            self.weatherWidget.updateWeatherLabels(currentLocation)
            self.mapWidget.updateMap(currentLocation)
            self.povertyWidget.updateLabels(currentLocation)
            self.descWidget.updateLabel(currentLocation)
            ...
            if currentLocation is not None:
                getWeather(location)
                self.storeComparisonData(currentLocation, location)            
                self.comparePage.updateLabels(currentLocation, location)                  
                self.stacked_widget.setCurrentWidget(self.comparePage)
                                
                mode = 'new'
                
                print(f"Showing comparison between {currentLocation.getAddress()} and {location.getAddress()}")
            else:
                print("Error: No current location to compare with")
                mode = 'new'
        ...
        def set_user_info(self, username, user_id, email):
